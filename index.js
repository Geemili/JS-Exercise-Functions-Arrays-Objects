...
*/
function getName(namedObj) {
  return `Hello, my name is ${namedObj.name}`;
}
...
function getGermanCars(inventory) {
    const isGerman = function(s) {
        return s === `Audi` || s === `Mercedes-Benz` || s === `Volkswagen` || s === `BMW`;
    };
    const germanCars = [];
    for (let i=0; i < inventory.length; i++) {
        let car = inventory[i];
        if (isGerman(car.car_make)) {
            germanCars.push(car);
        }
    }
    return germanCars;
